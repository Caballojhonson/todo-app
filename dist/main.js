...
/* harmony import */ var _storage__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! ./storage */ "./src/modules/storage.js");
/* harmony import */ var _objects__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(/*! ./objects */ "./src/modules/objects.js");
...
const DOM = (() => {
	...
	const calendar = (() => {
		...
		const renderCalendarTasks = (dayId) => {
			...
			for (let i = 0; i < taskArray.length; i++) {
				...
				_logic__WEBPACK_IMPORTED_MODULE_9__.logic.setPriorityCircleColor(
					_logic__WEBPACK_IMPORTED_MODULE_9__.logic.getUpcomingTasks()[i],
					priorityCircle
				);
	...
	const projects = (() => {
		const renderProjects = () => {
			...
			addBtn.src = _icons_add_png__WEBPACK_IMPORTED_MODULE_5__;
			const container = get('projectContainer');

			for (let i = 0; i < _storage__WEBPACK_IMPORTED_MODULE_10__.storage.projects.length; i++) {
				const card = createDiv('projectCard', container);

				createText(
					'h5',
					'projectDate',
					`${_storage__WEBPACK_IMPORTED_MODULE_10__.storage.projects[i].createdOn.day}`,
					card
				);
				createText('h4', 'projectTitle', `${_storage__WEBPACK_IMPORTED_MODULE_10__.storage.projects[i].name}`, card);
				createText(
					'h6',
					'projectDescription',
					`${_storage__WEBPACK_IMPORTED_MODULE_10__.storage.projects[i].description}`,
					card
				);
				createText(
					'h4',
					'completedPercentage',
					_logic__WEBPACK_IMPORTED_MODULE_9__.logic.generateProjectCompletionText(_storage__WEBPACK_IMPORTED_MODULE_10__.storage.projects[i]),
					card
				);
				createDiv('progressBarRed', card);
				let progressBar = createDiv('progressBarGreen', card);
				progressBar.style.width = `${_logic__WEBPACK_IMPORTED_MODULE_9__.logic.getCompletionPercent(
					_storage__WEBPACK_IMPORTED_MODULE_10__.storage.projects[i]
				)}`;
			}
		};
	...
	const eventHandling = (() => {
		...
		addNewProject.addEventListener('click', () => {
			(0,_objects__WEBPACK_IMPORTED_MODULE_11__.createProject)(
				get('projectTitleInput').value,
				get('projectDescInput').value
			);
			_storage__WEBPACK_IMPORTED_MODULE_10__.storage.loadArrays();
			let modal = get('newProjectPopup');
			modal.style.display = 'none';
				newProjectBtn.style.display = 'block';
...
const logic = (() => {
	...
	const generateProjectCompletionText = (project) => {
		...
		}else return `${logic.getCompletionPercent(_storage__WEBPACK_IMPORTED_MODULE_0__.storage.projects[i])} Complete`
	}
...
if (_modules_storage__WEBPACK_IMPORTED_MODULE_4__.storage.tasks.length > 0) setTimeout(() => _modules_DOM__WEBPACK_IMPORTED_MODULE_1__.DOM.overview.renderUpcoming(), 0);
...
setTimeout(() => _modules_DOM__WEBPACK_IMPORTED_MODULE_1__.DOM.projects.renderProjects(), 0);
