...
/* harmony import */ var _dateTime__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! ./dateTime */ "./src/modules/dateTime.js");
/* harmony import */ var _logic__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! ./logic */ "./src/modules/logic.js");
/* harmony import */ var _storage__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(/*! ./storage */ "./src/modules/storage.js");
/* harmony import */ var _objects__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! ./objects */ "./src/modules/objects.js");
...
const DOM = (() => {
	...
	const nav = (() => {
		...
		const setClock = () => {
			let clock = get('clock');
			clock.textContent = _dateTime__WEBPACK_IMPORTED_MODULE_9__.dateTime.getTime();
			setInterval(() => (clock.textContent = _dateTime__WEBPACK_IMPORTED_MODULE_9__.dateTime.getTime()), 1000);
		};

		const setDate = () => {
			let date = get('date');
			date.textContent = `${_dateTime__WEBPACK_IMPORTED_MODULE_9__.dateTime.current.day}`;
		};
	...
	const overview = (() => {
		const renderUpcoming = () => {
			...
			for (let i = 0; i < _logic__WEBPACK_IMPORTED_MODULE_10__.logic.getUpcomingTasks().length; i++) {
				let box = createDiv('boxes', container);

				let priorityBox = createDiv('priorityCircles', box);
				priorityBox.textContent = _logic__WEBPACK_IMPORTED_MODULE_10__.logic.getUpcomingTasks()[i].priority;
				_logic__WEBPACK_IMPORTED_MODULE_10__.logic.setPriorityCircleColor(_logic__WEBPACK_IMPORTED_MODULE_10__.logic.getUpcomingTasks()[i], priorityBox);
				...
				createText(
					'h6',
					'timeTill',
					_logic__WEBPACK_IMPORTED_MODULE_10__.logic.dueOrOverdue(
						_logic__WEBPACK_IMPORTED_MODULE_10__.logic.getUpcomingTasks()[i].deadline,
						_dateTime__WEBPACK_IMPORTED_MODULE_9__.dateTime.getTimeToUpcomingDeadlines()[i]
					),
					box
				);
				createText(
					'h4',
					'fromProject',
					_logic__WEBPACK_IMPORTED_MODULE_10__.logic.getUpcomingTasks()[i].fromProject,
					textContainer
				);
				createText(
					'h5',
					'taskTitle',
					_logic__WEBPACK_IMPORTED_MODULE_10__.logic.getUpcomingTasks()[i].name,
					textContainer
				);
			}
		};
		...
		const renderPending = () => {
			...
			createText(
				'h5',
				'pendingTasks',
				`${_logic__WEBPACK_IMPORTED_MODULE_10__.logic.countStats().pendingTaskCount} Tasks`,
				pendingContainer
			);
			createText(
				'h5',
				'pendingprojects',
				`${_logic__WEBPACK_IMPORTED_MODULE_10__.logic.countStats().pendingProjectCount} Projects`,
				pendingContainer
			);
			...
			createText(
				'h5',
				'pendingTasks',
				`${_logic__WEBPACK_IMPORTED_MODULE_10__.logic.countStats().completedTaskCount} Tasks`,
				completeContainer
			);
			createText(
				'h5',
				'pendingTasks',
				`${_logic__WEBPACK_IMPORTED_MODULE_10__.logic.countStats().completedProjectCount} Projects`,
				completeContainer
			);
		};
	...
	const calendar = (() => {
		...
		month.textContent = `${_dateTime__WEBPACK_IMPORTED_MODULE_9__.dateTime.thisMonth.monthAndYear}`;
		...
		const styleDeadlineDays = () => {
			...
			for (let i = 0; i < days.length; i++) {
				if (_logic__WEBPACK_IMPORTED_MODULE_10__.logic.dayHasDeadline(`${days[i].id} ${month.textContent}`)) {
					let priorityNum = _logic__WEBPACK_IMPORTED_MODULE_10__.logic.getHighestPriority(
						`${days[i].id} ${month.textContent}`
					);
					let dayToStyle = get(days[i].id);
					dayToStyle.style.borderColor = _logic__WEBPACK_IMPORTED_MODULE_10__.logic.getPriorityColor(priorityNum);
		...
		left.addEventListener('click', () => {
			...
			month.textContent = _dateTime__WEBPACK_IMPORTED_MODULE_9__.dateTime.substractMonth(currentMonth);
			currentMonth = month.textContent;
			renderCalendar(_dateTime__WEBPACK_IMPORTED_MODULE_9__.dateTime.generateCalendar(currentMonth));
		});

		right.addEventListener('click', () => {
			let currentMonth = month.textContent;
			month.textContent = _dateTime__WEBPACK_IMPORTED_MODULE_9__.dateTime.addMonth(currentMonth);
			currentMonth = month.textContent;
			renderCalendar(_dateTime__WEBPACK_IMPORTED_MODULE_9__.dateTime.generateCalendar(currentMonth));
		});
		...
		const renderCalendarTasks = (dayId) => {
			...
			const taskArray = _logic__WEBPACK_IMPORTED_MODULE_10__.logic.getTasksForDay(date);
			...
			for (let i = 0; i < taskArray.length; i++) {
				...
				priorityCircle.textContent = _logic__WEBPACK_IMPORTED_MODULE_10__.logic.getUpcomingTasks()[i].priority;
				_logic__WEBPACK_IMPORTED_MODULE_10__.logic.setPriorityCircleColor(
					_logic__WEBPACK_IMPORTED_MODULE_10__.logic.getUpcomingTasks()[i],
					priorityCircle
				);
				...
				createText(
					...
					`${_logic__WEBPACK_IMPORTED_MODULE_10__.logic.dueOrOverdue(
						task.deadline,
						_dateTime__WEBPACK_IMPORTED_MODULE_9__.dateTime.timeTill(task.deadline)
					)}`,
					timeTillBox
				);
	...
	const projects = (() => {
		const renderProjects = () => {
			...
			for (let i = 0; i < _storage__WEBPACK_IMPORTED_MODULE_11__.storage.projects.length; i++) {
				const card = createDiv('projectCard', container);
				card.id = _storage__WEBPACK_IMPORTED_MODULE_11__.storage.projects[i].index;
				
				createText('h4', 'projectTitle', `${_storage__WEBPACK_IMPORTED_MODULE_11__.storage.projects[i].name}`, card);
				createText(
					'h5',
					'projectDate',
					`${_storage__WEBPACK_IMPORTED_MODULE_11__.storage.projects[i].createdOn.day}`,
					card
				);
				...
				createText(
					'h6',
					'projectDescription',
					`${_storage__WEBPACK_IMPORTED_MODULE_11__.storage.projects[i].description}`,
					card
				);
				createText(
					'h4',
					'completedPercentage',
					_logic__WEBPACK_IMPORTED_MODULE_10__.logic.generateProjectCompletionText(_storage__WEBPACK_IMPORTED_MODULE_11__.storage.projects[i], i),
					card
				);
				createDiv('progressBarRed', card);
				let progressBar = createDiv('progressBarGreen', card);
				progressBar.style.width = `${_logic__WEBPACK_IMPORTED_MODULE_10__.logic.getCompletionPercent(
					_storage__WEBPACK_IMPORTED_MODULE_11__.storage.projects[i]
				)}`;
				...
				deleteBtn.addEventListener('click', (e) => {
					_logic__WEBPACK_IMPORTED_MODULE_10__.logic.removeItem(
						'project',
						e.target.parentNode.parentNode.id,
						_storage__WEBPACK_IMPORTED_MODULE_11__.storage.projects
					);
					renderProjects();
				});
	...
	const tasks = (() => {
		...
		const addNewTask = () => {
			...
			const fromProject = _storage__WEBPACK_IMPORTED_MODULE_11__.storage.projects[projectId].name;
			...
			(0,_objects__WEBPACK_IMPORTED_MODULE_12__.createTask)(
				`${name}`,
				`${description}`,
				`${dueDate}`,
				`${isChecked()}`,
				...
				_storage__WEBPACK_IMPORTED_MODULE_11__.storage.projects[projectId].tasks.push(
					_storage__WEBPACK_IMPORTED_MODULE_11__.storage.tasks[_storage__WEBPACK_IMPORTED_MODULE_11__.storage.tasks.length - 1]
				);
				
				_storage__WEBPACK_IMPORTED_MODULE_11__.storage.localStore();
				projects.renderProjects();
				overview.renderUpcoming();
				overview.renderPending();
				calendar.renderCalendar(_dateTime__WEBPACK_IMPORTED_MODULE_9__.dateTime.generateCalendar(new Date()));
		};
	...
	const eventHandling = (() => {
		...
		addNewProject.addEventListener('click', () => {
			(0,_objects__WEBPACK_IMPORTED_MODULE_12__.createProject)(
				get('projectTitleInput').value,
				get('projectDescInput').value
			);
			projects.renderProjects();
			console.log(_storage__WEBPACK_IMPORTED_MODULE_11__.storage.projects);
		...
		const populateDate = () => {
			...
			dateInput.value = _dateTime__WEBPACK_IMPORTED_MODULE_9__.dateTime.current.day;
		};
